...
# @router.get('/')
def get_all_reviews_for_a_book(
    id: int,
    db: Session = Depends(get_db),
):
    results = db.query(models.Review).filter(
        models.Review.book_id == id
    ).all()
    return results
...
def delete_review(
    ...
    if current_user.user_profile_id != review.given_by:
        raise HTTPException(
...
def update_review(
    ...
    updated_review_data = updated_review.dict()
    updated_review_data["updated_at"] = datetime.now().astimezone()

    # print(updated_review_data)
    review_query.update(
        updated_review_data,
